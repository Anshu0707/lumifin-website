...
export default function FAQSection() {
  ...
  const landingFaqs = getFaqs(t).slice(0, 5);

  return (
    <section id="faq" className="py-32 bg-white reveal active">
      <div className="max-w-4xl mx-auto px-8">
        <div className="text-center mb-20 space-y-6">
          <span className="text-primary font-bold text-xs tracking-widest uppercase">{t('faqSection.eyebrow')}</span>
          <h2 className="text-6xl font-black tracking-tighter text-slate-900">{t('faqSection.title')}</h2>
          <p className="text-slate-500 text-xl font-medium">{t('faqSection.subtitle')}</p>
        </div>
